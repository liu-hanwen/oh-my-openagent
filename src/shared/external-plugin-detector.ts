/**
 * Detects external plugins that may conflict with oh-my-openquant features.
...
/**
 * Known notification plugins that conflict with oh-my-openquant's session-notification.
...
export function getNotificationConflictWarning(pluginName: string): string {
  return `[oh-my-openquant] External notification plugin detected: ${pluginName}

Both oh-my-openquant and ${pluginName} listen to session.idle events.
   Running both simultaneously can cause crashes on Windows.

   oh-my-openquant's session-notification has been auto-disabled.

   To use oh-my-openquant's notifications instead, either:
   1. Remove ${pluginName} from your opencode.json plugins
   2. Or set "notification": { "force_enable": true } in oh-my-openquant.json`
}
