...
function resetTestCache(): void {
  ...
  mkdirSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-openquant"), { recursive: true })
  writeFileSync(
    join(TEST_OPENCODE_CACHE_DIR, "package.json"),
    JSON.stringify({ dependencies: { "oh-my-openquant": "latest", other: "1.0.0" } }, null, 2)
  )
  writeFileSync(
    ...
    JSON.stringify(
      {
        workspaces: {
          "": {
            dependencies: { "oh-my-openquant": "latest", other: "1.0.0" },
          },
        },
        packages: {
          "oh-my-openquant": {},
  ...
  writeFileSync(
    join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-openquant", "package.json"),
...
describe("invalidatePackage", () => {
  ...
  it("invalidates the installed package from the OpenCode cache directory", async () => {
    ...
    expect(existsSync(join(TEST_OPENCODE_CACHE_DIR, "node_modules", "oh-my-openquant"))).toBe(false)

    const packageJson = JSON.parse(readFileSync(join(TEST_OPENCODE_CACHE_DIR, "package.json"), "utf-8")) as {
      dependencies?: Record<string, string>
    }
    expect(packageJson.dependencies?.["oh-my-openquant"]).toBe("latest")
    ...
    expect(bunLock.workspaces?.[""]?.dependencies?.["oh-my-openquant"]).toBe("latest")
    expect(bunLock.workspaces?.[""]?.dependencies?.other).toBe("1.0.0")
    expect(bunLock.packages?.["oh-my-openquant"]).toBeUndefined()
