...
mock.module("../constants", () => ({
  ...
  NPM_REGISTRY_URL: "https://registry.npmjs.org/-/package/oh-my-openquant/dist-tags",
  ...
  INSTALLED_PACKAGE_JSON: join(TEST_CACHE_DIR, "node_modules", "oh-my-openquant", "package.json"),
...
function resetTestCache(currentVersion = "3.10.0"): void {
  ...
  writeFileSync(
    ...
    JSON.stringify({ dependencies: { "oh-my-openquant": currentVersion, other: "1.0.0" } }, null, 2)
  )
}
...
function readCachePackageJsonVersion(): string | undefined {
  ...
  return pkg.dependencies?.["oh-my-openquant"]
}
...
describe("syncCachePackageJsonToIntent", () => {
  ...
  describe("#given write permission denied", () => {
    it("#then returns write_error", async () => {
      ...
      writeFileSync(
        join(TEST_CACHE_DIR, "package.json"),
        JSON.stringify({ dependencies: { "oh-my-openquant": "3.10.0" } }, null, 2)
      )
