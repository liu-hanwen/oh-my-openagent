...
export function formatVersionOutput(info: VersionInfo): string {
  ...
  lines.push("")
  lines.push(color.bold(color.white("oh-my-openquant Version Information")))
  ...
  switch (info.status) {
    ...
    case "outdated":
      ...
      lines.push(`  ${color.dim("Run:")} ${color.cyan("cd ~/.config/opencode && bun update oh-my-openquant")}`)
      break
    case "local-dev":
