...
24 schema files composing `OhMyOpenQuantConfigSchema`. Zod v4 validation with `safeParse()`. All fields optional — omitted fields use plugin defaults.

## SCHEMA TREE

```
config/schema/
├── oh-my-openquant-config.ts    # ROOT: OhMyOpenQuantConfigSchema (composes all below)
...
2. Add field to `oh-my-openquant-config.ts` root schema
