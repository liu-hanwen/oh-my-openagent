  ...
  return tool({
    ...
    async execute(args: CallOmoAgentArgs, toolContext) {
      ...
      if (disabledAgents.some((disabled) => disabled.toLowerCase() === normalizedAgent)) {
        return `Error: Agent "${normalizedAgent}" is disabled via disabled_agents configuration. Remove it from disabled_agents in your oh-my-openquant.json to use it.`
      }
