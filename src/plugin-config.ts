...
export function parseConfigPartially(
...
): OhMyOpenQuantConfig | null {
  const fullResult = OhMyOpenQuantConfigSchema.safeParse(rawConfig);
  ...
  for (const key of Object.keys(rawConfig)) {
    ...
    const sectionResult = OhMyOpenQuantConfigSchema.safeParse({ [key]: rawConfig[key] });
  ...
  return partialConfig as OhMyOpenQuantConfig;
}
...
export function loadConfigFromPath(
  ...
  try {
    if (fs.existsSync(configPath)) {
      ...
      const result = OhMyOpenQuantConfigSchema.safeParse(rawConfig);
...
export function mergeConfigs(
  base: OhMyOpenQuantConfig,
  override: OhMyOpenQuantConfig
): OhMyOpenQuantConfig {
  return {
    ...base,
    ...override,
...
export function loadPluginConfig(
  ...
  const userBasePath = path.join(configDir, "oh-my-openquant");
  ...
  const projectBasePath = path.join(directory, ".opencode", "oh-my-openquant");
  ...
  let config: OhMyOpenQuantConfig =
