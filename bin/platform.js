 ...
 * @returns {string} Package name like "oh-my-openquant-darwin-arm64"
...
export function getPlatformPackage({ platform, arch, libcFamily }) {
  ...
  return `oh-my-openquant-${os}-${arch}${suffix}`;
}
...
function getBaselinePlatformPackage({ platform, arch, libcFamily }) {
  ...
  if (platform === "darwin") {
    return "oh-my-openquant-darwin-x64-baseline";
  }

  if (platform === "win32") {
    return "oh-my-openquant-windows-x64-baseline";
  }
  ...
  if (platform === "linux") {
    ...
    if (libcFamily === "musl") {
      return "oh-my-openquant-linux-x64-musl-baseline";
    }

    return "oh-my-openquant-linux-x64-baseline";
  }
 ...
 * @returns {string} Relative path like "oh-my-openquant-darwin-arm64/bin/oh-my-openquant"
 */
export function getBinaryPath(pkg, platform) {
  const ext = platform === "win32" ? ".exe" : "";
  return `${pkg}/bin/oh-my-openquant${ext}`;
}
