...
const PACKAGE_NAME = "oh-my-openquant"
...
console.log("=== Publishing oh-my-openquant (multi-package) ===\n")
...
async function updateAllPackageVersions(newVersion: string): Promise<void> {
  ...
  for (const platform of PLATFORM_PACKAGES) {
    const pkgName = `oh-my-openquant-${platform}`
...
async function gitTagAndRelease(newVersion: string, notes: string[]): Promise<void> {
  ...
  await $`git add package.json assets/oh-my-openquant.schema.json`
  for (const platform of PLATFORM_PACKAGES) {
