...
First off, thanks for taking the time to contribute! This document provides guidelines and instructions for contributing to oh-my-openquant.
...
2. **Update your OpenCode config** (`~/.config/opencode/opencode.json` or `opencode.jsonc`):
   ...
   ```json
   {
     "plugin": ["file:///absolute/path/to/oh-my-openquant/dist/index.js"]
   }
   ```

   For example, if your project is at `/Users/yourname/projects/oh-my-openquant`:

   ```json
   {
     "plugin": ["file:///Users/yourname/projects/oh-my-openquant/dist/index.js"]
   }
   ```

   > **Note**: Remove `"oh-my-openquant"` from the plugin array if it exists, to avoid conflicts with the npm version.
...
oh-my-openquant/
├── src/
│   ├── index.ts         # Plugin entry (OhMyOpenQuantPlugin)
