...
Complete reference for `oh-my-openquant.jsonc` configuration. This document covers every available option with examples.
...
1. `.opencode/oh-my-openquant.jsonc` / `.opencode/oh-my-openquant.json`
2. User config (`.jsonc` preferred over `.json`):

| Platform    | Path                                      |
| ----------- | ----------------------------------------- |
| macOS/Linux | `~/.config/opencode/oh-my-openquant.jsonc` |
| Windows     | `%APPDATA%\opencode\oh-my-openquant.jsonc` |
...
```json
{
  "$schema": "https://raw.githubusercontent.com/code-yeongyu/oh-my-openagent/dev/assets/oh-my-openquant.schema.json"
}
```

Run `bunx oh-my-openquant install` for guided setup. Run `opencode models` to list available models.
...
Run `bunx oh-my-openquant doctor --verbose` to see effective model resolution for your config.
