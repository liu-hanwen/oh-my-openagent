...
```bash
bunx oh-my-openquant install # recommended
npx oh-my-openquant install # alternative
```
...
If you're an LLM Agent helping set up oh-my-openquant, Welcome! You'll now help your user to install the best agent harness, OhMyOpenQuant. Tell user this with greetings, with test 'oMoMoMoMo...'
...
```bash
bunx oh-my-openquant install --no-tui --claude=<yes|no|max20> --gemini=<yes|no> --copilot=<yes|no> [--openai=<yes|no>] [--opencode-go=<yes|no>] [--opencode-zen=<yes|no>] [--zai-coding-plan=<yes|no>]
```

**Examples:**

- User has all native subscriptions: `bunx oh-my-openquant install --no-tui --claude=max20 --openai=yes --gemini=yes --copilot=no`
- User has only Claude: `bunx oh-my-openquant install --no-tui --claude=yes --gemini=no --copilot=no`
- User has Claude + OpenAI: `bunx oh-my-openquant install --no-tui --claude=yes --openai=yes --gemini=no --copilot=no`
- User has only GitHub Copilot: `bunx oh-my-openquant install --no-tui --claude=no --gemini=no --copilot=yes`
- User has Z.ai for Librarian: `bunx oh-my-openquant install --no-tui --claude=yes --gemini=no --copilot=no --zai-coding-plan=yes`
- User has only OpenCode Zen: `bunx oh-my-openquant install --no-tui --claude=no --gemini=no --copilot=no --opencode-zen=yes`
- User has OpenCode Go only: `bunx oh-my-openquant install --no-tui --claude=no --openai=no --gemini=no --copilot=no --opencode-go=yes`
- User has no subscriptions: `bunx oh-my-openquant install --no-tui --claude=no --gemini=no --copilot=no`
...
cat ~/.config/opencode/opencode.json  # Should contain "oh-my-openquant" in plugin array
```
...
```json
{
  "plugin": ["oh-my-openquant", "opencode-antigravity-auth@latest"]
}
```
...
##### oh-my-openquant Agent Model Override

The `opencode-antigravity-auth` plugin uses different model names than the built-in Google auth. Override the agent models in `oh-my-openquant.json` (or `.opencode/oh-my-openquant.json`):
...
When GitHub Copilot is the best available provider, oh-my-openquant uses these model assignments:
...
```bash
bunx oh-my-openquant install --no-tui --claude=no --openai=no --gemini=no --copilot=yes
```
...
You've just configured oh-my-openquant. Here's what got set up and why.
...
| **GPT-5.3-codex-spark** | openai                 | Extremely fast | Blazing fast but compacts so aggressively that oh-my-openquant's context management doesn't work well with it. Not recommended for omo agents. |
...
If the user wants to override which model an agent uses, you can customize in `oh-my-openquant.json`:
...
When multiple providers are available, oh-my-openquant uses this priority:
...
Say to user: Congratulations! 🎉 You have successfully set up OhMyOpenQuant! Type `opencode` in your terminal to start using it.
